...
// ─── Canvas size helper (single source of truth) ─────────────────
function calcCanvasSize(containerWidth: number) {
  const cw = containerWidth;
  const ch = Math.min(containerWidth * 0.75, window.innerHeight * 0.55);
  return { cw, ch };
}
...
export default function Home() {
  ...
  // Drag precision: track start position for dampened drag
  ...
  function getCanvasTransform() {
    ...
    const { cw, ch } = calcCanvasSize(containerRef.current.clientWidth);
  ...
  const drawCanvas = useCallback(() => {
    ...
    const { cw, ch } = calcCanvasSize(container.clientWidth);
    ...
    if (points.length > 0) {
      ...
      // Edge labels: show which edge is width vs height
      if (points.length === 4) {
        const edges = [
  ...
  const drawLoupe = useCallback(
    (imgX: number, imgY: number, screenX: number, screenY: number) => {
      ...
      ctx.drawImage(image, sx, sy, srcSize, srcSize, 0, 0, loupeSize, loupeSize);
      ...
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(loupeSize / 2, loupeSize / 2, loupeSize / 2 - 1.5, 0, Math.PI * 2);
      ctx.stroke();
  ...
  const getCanvasPosFromClient = (clientX: number, clientY: number): Point | null => {
    const canvas = canvasRef.current;
  ...
  const handlePointerDown = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    if (nearIdx >= 0) {
      ...
      drawLoupe(points[nearIdx].x, points[nearIdx].y, clientX, clientY);
      e.preventDefault();
      return;
    }
    ...
    if (points.length < 4) {
      const t = getCanvasTransform()!;
      const imgPos = canvasToImg(canvasPos.x, canvasPos.y, t.scale, t.ox, t.oy);
  ...
  const handlePointerMove = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    if (dragStartScreen.current && dragStartImg.current) {
      ...
      const newX = Math.max(0, Math.min(dragStartImg.current.x + imgDx, image.width - 1));
      const newY = Math.max(0, Math.min(dragStartImg.current.y + imgDy, image.height - 1));
        ...
        try {
          ...
          const avgSrcWidth =
            (Math.hypot(srcPoints[1].x - srcPoints[0].x, srcPoints[1].y - srcPoints[0].y) +
              Math.hypot(srcPoints[2].x - srcPoints[3].x, srcPoints[2].y - srcPoints[3].y)) / 2;
          const avgSrcHeight =
            (Math.hypot(srcPoints[3].x - srcPoints[0].x, srcPoints[3].y - srcPoints[0].y) +
              Math.hypot(srcPoints[2].x - srcPoints[1].x, srcPoints[2].y - srcPoints[1].y)) / 2;

          const pxPerCm = Math.max(avgSrcWidth / realW, avgSrcHeight / realH);
            ...
            for (let x = 0; x < outW; x++) {
              ...
              if (
                ...
                const [r, g, b, a] = bilinearInterpolate(
                  srcData.data, image.width, image.height, srcPt.x, srcPt.y
                );
  ...
  return (
    <main className="flex flex-col items-center min-h-screen px-3 py-4 max-w-2xl mx-auto">
      <h1 className="text-xl font-bold tracking-tight mb-1 font-mono text-foreground">
        PerspektivKor <span className="text-xs font-normal text-text-dim">v0.7</span>
      </h1>
        ...
        {stepLabels.map((s, i) => (
          <div key={s.n} className="flex items-center flex-1">
            <div className="flex flex-col items-center flex-1">
              ...
              <span className="text-[10px] mt-1 text-text-dim">{s.label}</span>
            </div>
      ...
      {step === 1 && (
        ...
        >
          ...
          />
          <div className="mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-14 h-14 mx-auto text-text-dim" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6.827 6.175A2.31 2.31 0 015.186 7.23c-.38.054-.757.112-1.134.175C2.999 7.58 2.25 8.507 2.25 9.574V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9.574c0-1.067-.75-1.994-1.802-2.169a47.865 47.865 0 00-1.134-.175 2.31 2.31 0 01-1.64-1.055l-.822-1.316a2.192 2.192 0 00-1.736-1.039 48.774 48.774 0 00-5.232 0 2.192 2.192 0 00-1.736 1.039l-.821 1.316z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 12.75a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0z" />
            </svg>
          </div>
          <p className="text-foreground font-medium mb-1">Tapni za kameru ili odaberi fotku</p>
          <p className="text-xs text-text-dim">Drag &amp; drop ili klikni za galeriju</p>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="w-full mt-4 bg-surface rounded-xl p-5">
          ...
          <div className="flex gap-3 items-end">
            ...
            <span className="text-text-dim pb-2.5 font-mono text-lg">x</span>
            <div className="flex-1">
